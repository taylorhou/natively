...
def _w600_replace(path, data: bytes):
    ...
    the new, never a truncated one in between."""
    tmp = path + ".tmp"
    fd = os.open(tmp, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
    try:
        with os.fdopen(fd, "wb") as f:
        ...
        raise
    os.replace(tmp, path)
    _fsync_dir(os.path.dirname(path))  # the stored envelope is on disk before it is posted: a restart never re-mints it

"""Backport branch (98e47a8-based): the flush's send phase posts on
...
def _item(f, control=False):
    return (f, "/unused/" + f, {"to": "ed25519:rcpt", "control": True} if control else {"to": "ed25519:rcpt"},
            {"msg_id": "msg_" + f}, "fp")


def test_a_control_429_defers_the_rest_of_its_shard(tmp_path, hub, principal):
    n1 = make_node(tmp_path, "n1", hub.url, principal, {"alpha": ["msg.send"]})
    posts = []
    ...
    n1._hub_req = full
    res = n1._flush_post_shard([_item("ctl_a.json", control=True), _item("out_b.json"), _item("out_c.json")])
